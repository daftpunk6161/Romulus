...
| D6 | Glow-Effekte für Retro-Themes (optional, ReduceMotion-aware) | Delight | Mittel | ✅ erledigt — `EffectAccentGlow`-Resource in allen 7 Themes definiert (Retro-Glow mit passender Akzentfarbe für SynthwaveDark/RetroCRT/ArcadeNeon, transparente Dummy-Effect für Light/CleanDarkPro/CleanDaylight/HighContrast zur einheitlichen Key-Auflösung). CommandBar-Logo (`CommandBar.xaml`) bindet den Effect via `DynamicResource` mit `DataTrigger` auf `Shell.ReduceMotion=true` → `{x:Null}`. Referenz-Pattern weiterhin in `WizardView.xaml`. Invarianten: `D6_AllThemes_DefineEffectAccentGlow`, `D6_CommandBar_LogoRespectsReduceMotion`, `D6_WizardView_AnimationsRespectReduceMotion`. |
| D7 | Tool-Documentation-Drawer | Power-User | Hoch | ✅ erledigt — `ToolsViewModel` exponiert `SelectedToolDoc` / `IsToolDocOpen` / `ShowToolDocCommand` / `CloseToolDocCommand`. Info-Button (`\xE946`) in `ToolCardTemplate` öffnet einen 380px breiten Drawer-Overlay rechts in `ToolsView.xaml` mit Sections für Beschreibung / Reifegrad / Empfehlung / Verfügbarkeit und einem „Werkzeug starten"-Shortcut. `ExecuteTool` schließt den Drawer automatisch, wenn das gestartete Tool gerade angezeigt wird. Invarianten: `D7_ToolsViewModel_HasToolDocCommands`, `D7_ToolsView_HasToolDocDrawer`, `D7_ExecuteTool_ClosesDrawer`. |
