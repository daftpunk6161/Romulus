...
public sealed class HoldoutGateTests : IClassFixture<BenchmarkFixture>
{
    ...
    public void Holdout_DetectionRate_AboveMinimum()
    {
        ...
        // Holdout has many entries without stub definitions (negative controls, wrong-folder, etc.)
        // that generate as empty/zero-byte files → detection fails as expected.
        // Threshold is intentionally low: the drift test (above) is the real overfitting check.
