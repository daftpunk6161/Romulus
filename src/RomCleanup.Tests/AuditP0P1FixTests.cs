...
public sealed class AuditP0P1FixTests : IDisposable
{
    ...
    // ═══════════════════════════════════════════════════════════════════
    //  P1-5: Stub-Commands should have CanExecute = false
    // ═══════════════════════════════════════════════════════════════════
    ...
    public void StubCommand_CanExecute_ShouldBeFalse(string commandKey)
    {
        var dialog = new MinimalDialog();
        var settings = new MinimalSettings();
        var windowHost = new MinimalWindowHost();
        var vm = new MainViewModel(new MinimalTheme(), dialog, settings);
        var sut = new FeatureCommandService(vm, settings, dialog, windowHost);
        sut.RegisterCommands();

        Assert.True(vm.FeatureCommands.ContainsKey(commandKey),
            $"Command '{commandKey}' not registered");

        var command = vm.FeatureCommands[commandKey];
        Assert.False(command.CanExecute(null),
            $"Stub command '{commandKey}' should NOT be executable (CanExecute must be false)");
    }
